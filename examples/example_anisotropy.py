...
im = ax.contourf(X, Z, LFP, 51, vmin=-np.max(np.abs(LFP)) / 2, vmax=np.max(np.abs(LFP)) / 2,
           cmap='bwr',
           zorder=-2)
...
im = ax2.contourf(X, Z, LFP, 51, vmin=-np.max(np.abs(LFP)) / 2, vmax=np.max(np.abs(LFP)) / 2,
           cmap='bwr',
           zorder=-2)
cbar = plt.colorbar(im)
