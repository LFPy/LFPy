...
class ElectrodeDetermineCoeffs(Electrode):
    ...
    def _import_c(self, cell):
        ...
        if str(type(cell))[:17] == "<class 'LFPy.cell.Cell'>"[:17] or \
                    str(type(cell))[:17] == "<class 'cellwithelectrode.CellWithElectrode'>"[:17]:
            self.c[0] = self.cell()
            for v in variables:
                setattr(self.c[0], v, getattr(cell, v))
        elif type(cell) == dict:
            for k in cell:
                if str(type(cell[k]))[:17] != "<class 'LFPy.cell.Cell'>"[:17]:
